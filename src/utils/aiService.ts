...
export class AIService {
    ...
    private static readonly HUGGINGFACE_API_URL =
    ...
    private static readonly COHERE_API_URL =
    ...
    static async generateQuestions(
      ...
      Return ONLY the questions, one per line, without numbering or bullet points.`;
            ...
            if (!result.success) {
                console.log(
                    "AI generation failed, using sample questions:",
                    result.error
                );
        ...
        try {
            ...
            for (const model of models) {
                try {
                    ...
                    console.log(
                        `Success with model ${model}, generated text:`,
                        generatedText
                    );
                    const questions =
                        this.parseGeneratedQuestions(generatedText);
                ...
                messages: [
                    {
                        role: "system",
                        content:
                            "You are an expert at creating engaging conversation questions for dating and social apps.",
                    },
    ...
    private static parseGeneratedQuestions(text: string): string[] {
        ...
        if (questions.length === 0) {
            questions = text
                ...
                .filter(
                    (sentence) => sentence.length > 10 && sentence.length < 200
                )
                .slice(0, 5);
        }
    ...
    static getAvailableProviders(): Array<{
        ...
        return [
            {
                id: "huggingface",
                name: "Hugging Face (Free) - Try This First",
                free: true,
                requiresKey: true,
            },
            {
                id: "openrouter",
                name: "OpenRouter (Free tier) - Recommended",
                free: true,
                requiresKey: true,
            },
            {
