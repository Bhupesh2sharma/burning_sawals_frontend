...
export default function AdminPage() {
    ...
    const [activeAnalyticsTab, setActiveAnalyticsTab] = useState<
        "overview" | "questions" | "users" | "top"
    >("overview");
    const [analyticsSortBy, setAnalyticsSortBy] = useState<
        | "total_interactions"
        | "likes"
        | "super_likes"
        | "dislikes"
        | "created_at"
    >("total_interactions");
    const [analyticsSortOrder, setAnalyticsSortOrder] = useState<
        "asc" | "desc"
    >("desc");
    ...
    const handleAddAIGeneratedQuestion = async (
        question: string,
        prompt: string,
        genreIds: number[]
    ) => {
        ...
        try {
            ...
            setAddQuestionStatus({
                error: "Failed to add AI-generated question.",
            });
        }
    };
    ...
    const handleRenameType = async (id: string) => {
        ...
        try {
            ...
            setQuestionTypes((qts) =>
                qts.map((qt) =>
                    qt.id === id ? { ...qt, type_name: renameTypeName } : qt
                )
            );
    ...
    const handleDeleteType = async (type_id: string) => {
        ...
        try {
            ...
            setQuestionTypes((qts) =>
                qts.filter((qt) => qt.type_id !== type_id)
            );
            ...
            setGenres((gs) =>
                gs.map((g) =>
                    g.genre_id === id
                        ? {
                              ...g,
                              name: renameGenreName,
                              genre_name: renameGenreName,
                          }
                        : g
                )
            );
    ...
    const fetchAnalyticsData = async () => {
        ...
        try {
            ...
            const [
                ...
                AnalyticsService.getAllQuestionsWithAnalytics(
                    token,
                    1,
                    50,
                    analyticsSortBy,
            ...
            if (questionsResponse.status === "fulfilled") {
                questionsData =
                    questionsResponse.value.data?.data?.items ||
                    questionsResponse.value.data?.data ||
                    [];
            }
            ...
            if (userResponse.status === "fulfilled") {
                const userData =
                    userResponse.value.data?.data || userResponse.value.data;
                if (userData) {
                    userAnalyticsData = {
            ...
            if (allUsersResponse.status === "fulfilled") {
                allUsersData =
                    allUsersResponse.value.data?.data ||
            ...
            if (topQuestionsResponse.status === "fulfilled") {
                const responseData =
                    topQuestionsResponse.value.data?.data ||
                    topQuestionsResponse.value.data;
                topQuestionsData = Array.isArray(responseData)
                    ? responseData
                    : responseData?.items || [];
            }
            ...
            if (statsResponse.status === "fulfilled") {
                systemStatsData = {
                    ...
                    total_questions:
                        statsResponse.value.data?.total_questions || 0,
                    total_interactions:
                        statsResponse.value.data?.total_interactions || 0,
                    daily_active_users:
                        statsResponse.value.data?.daily_active_users || 0,
                    system_health:
                        healthResponse.status === "fulfilled"
                            ? healthResponse.value.data?.status || "unknown"
                            : "unknown",
                    database_status:
                        healthResponse.status === "fulfilled"
                            ? healthResponse.value.data?.database || "unknown"
                            : "unknown",
                };
            }
    ...
    return (
        <div className="min-h-screen bg-gray-100 flex flex-col">
            {/* Header */}
            <div className="bg-pink-200 py-2 px-4 text-center font-medium text-sm border">
                Hey, Admin How you doing today ?
            </div>
          ...
          >
            Users
                    ...
                    <button
                        className={`text-left px-6 py-2 text-gray-700 hover:bg-gray-100 ${
                            selectedSection === "Add Questions"
                                ? "font-semibold bg-gray-100 border-l-4 border-pink-400"
                                : ""
                        }`}
                        ...
                        <>
                            ...
                            <div className="flex gap-2 mb-6 border-b pb-2">
                                <button
                                    ...
                                    onClick={() =>
                                        setActiveAdminTab("add-question-type")
                                    }
                                >
                                    Add Question Type
                                </button>
                                <button
                                    className={`px-4 py-2 rounded-t font-medium ${
                                        activeAdminTab === "view-question-types"
                                            ? "bg-pink-400 text-white"
                                            : "bg-gray-200 text-gray-700 hover:bg-gray-300"
                                    }`}
                                    onClick={async () => {
                                ...
                                <button
                                    ...
                                    onClick={() =>
                                        setActiveAdminTab("ai-generator")
                                    }
                                >
                                    🤖 AI Generator
                                </button>
                                ...
                                <div className="mb-4">
                                    ...
                                    >
                                        ...
                                        <div className="flex gap-2">
                                            <button
                                                type="submit"
                                                className="bg-pink-400 text-white px-4 py-1 rounded hover:bg-pink-600 transition cursor-pointer"
                                            >
                                                Submit
                                            </button>
                                        </div>
                                    ...
                                    <div className="overflow-x-auto bg-white rounded shadow mt-8">
                                        ...
                                        {questionEditStatus.success && (
                                            <div className="text-green-600 text-sm mt-1">
                                                {questionEditStatus.success}
                                            </div>
                                        )}
                                        {questionEditStatus.error && (
                                            <div className="text-red-600 text-sm mt-1">
                                                {questionEditStatus.error}
                                            </div>
                                        )}
                                    ...
                                    <form
                                        ...
                                        onSubmit={async (e) => {
                                            ...
                                            try {
                                                ...
                                                setAddTypeStatus({
                                                    error: "Failed to add question type.",
                                                });
                                            }
                                        }}
                                    >
                                                        ...
                                                        <td className="px-4 py-2 align-top">
                                                            ...
                                                            qt.type_id ? (
                                                                ...
                                                                >
                                                                    ...
                                                                    <button
                                                                        type="submit"
                                                                        className="text-pink-300 font-medium cursor-pointer"
                                                                    >
                                                                        Save
                                                                    </button>
                                                                    <button
                                ...
                                <div className="mb-4">
                                    ...
                                    >
                                        ...
                                        <div className="flex gap-2">
                                            <button
                                                type="submit"
                                                className="bg-pink-400 text-white px-4 py-1 rounded hover:bg-green-600 transition cursor-pointer"
                                            >
                                                Submit
                                            </button>
                                            <button
                                                type="button"
                                                className="bg-gray-300 text-gray-700 px-4 py-1 rounded hover:bg-gray-400 transition cursor-pointer"
                                                onClick={() =>
                                                    setActiveAdminTab(
                                                        "add-question"
                                                    )
                        ...
                        <div className="bg-white rounded shadow">
                            ...
                            <div className="border-b">
                                <nav className="flex">
                                    ...
                                    <button
                                        onClick={() =>
                                            setActiveAnalyticsTab("questions")
                                        }
                                        className={`px-6 py-3 text-sm font-medium ${
                                            ...
                                            <div>
                                                <div className="flex justify-between items-center mb-4">
                                                    ...
                                                    <div className="flex gap-2">
                                                        ...
                                                        >
                                                            <option value="total_interactions">
                                                                Total
                                                                Interactions
                                                            </option>
                                                            <option value="likes">
