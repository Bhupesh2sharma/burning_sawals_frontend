    ...
    <div className="flex flex-col bg-[#feedf2]">
      ...
      <div className="relative w-full max-w-md mx-auto mt-4 mb-2" style={{ minHeight: 48 }}>
        ...
        <h1 className="text-2xl font-libre-bodoni text-black text-center absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 flex items-center gap-2 justify-center w-max">
          {selectedQuestionType ? selectedQuestionType.type_name : "Question Types"} <span>🧑‍🤝‍🧑</span>
        </h1>
        {/* Right side - Feedback & Suggestions text and Logout button */}
        <div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-3">
          <span className="text-xs font-libre-bodoni text-gray-600">feedback & suggestions</span>
          <button 
            onClick={logout}
            className="text-xs font-libre-bodoni text-gray-600 hover:text-gray-800 transition-colors"
          >
            logout
          </button>
        </div>
      ...
      <div className="flex flex-col items-center max-w-md mx-auto bg-[#E7E7E9] rounded-2xl p-4 mb-2">
        ...
        {!selectedQuestionType ? (
          <div className="w-full">
            <h2 className="text-lg font-semibold text-center mb-4">Select a Question Type</h2>
            <div className="grid grid-cols-1 gap-2">
              {questionTypes.map((questionType) => (
                <button
                  key={questionType.type_id}
                  onClick={() => setSelectedQuestionType(questionType)}
                  className="p-3 bg-white rounded-lg text-left hover:bg-gray-50 transition"
                >
                  <div className="font-medium text-gray-800">{questionType.type_name}</div>
                  <div className="text-sm text-gray-500">
                    {questionType.genres?.length || 0} genres available
                  </div>
                </button>
              ))}
            </div>
          ...
          <>
            ...
            <div className="flex gap-2 justify-center mb-4 w-full max-w-xs flex-wrap">
              {selectedQuestionType.genres?.map((genre: any) => (
                <button
                  key={genre.genre_id}
                  className={`flex-1 bg-[#CB2655] text-white rounded-lg px-4 py-2 text-xs font-semibold shadow font-sans min-w-0 ${selectedGenreId === genre.genre_id ? 'ring-2 ring-pink-400' : ''}`}
            ...
            {!selectedGenreId && (
              <div className="text-center py-8">
                <p className="text-gray-600">Select a genre above to view questions.</p>
              </div>
            )}
          </>
        )}
