...
export default function FriendsPage() {
  const [friendGenres, setFriendGenres] = useState<any[]>([]);
  const [selectedGenreId, setSelectedGenreId] = useState<number | null>(null);
  const [questions, setQuestions] = useState<any[]>([]);
  const [loading, setLoading] = useState(false);
  const [currentQuestionIdx, setCurrentQuestionIdx] = useState(0);

  // Fetch genres for 'For Friends' question type
  useEffect(() => {
    const fetchFriendGenres = async () => {
      const allTypes = await getAllQuestionTypes();
      const types = allTypes.data || allTypes;
      const friendType = types.find((t: any) =>
        t.type_name.toLowerCase().includes("friend")
      );
      if (friendType && Array.isArray(friendType.genres)) {
        setFriendGenres(friendType.genres);
        if (friendType.genres.length > 0) {
          setSelectedGenreId(friendType.genres[0].genre_id);
        }
      }
    };
    fetchFriendGenres();
